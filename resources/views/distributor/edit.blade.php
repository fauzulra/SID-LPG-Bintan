...
@section('styles')
    ...
    <style>
        .bg-gradient {
            background: linear-gradient(to right, #b3cde0, #fceabb);
            !important;
            height: 60px;
            /* atur tinggi banner */
        }
...
@section('content')
    <div class="col-xl-12 col-lg-7">
        <div class="card shadow mb-4 " style="border-radius: 2rem; !important">
            ...
            <div class="card-header p-4 bg-gradient"
                    ...
                    <div class="col-12">
                        <div class="d-flex align-items-center justify-content-between ">
                            <div class="d-flex align-items-center">
                                ...
                                <div class="ms-3">
                                    <h5 class="mb-0 ml-3 text-gray-800">{{ $distributor->user->name }}</h5>
                                    <small class="text-muted ml-3 text-gray-800">{{ $distributor->user->email }}</small>
                                </div>
                            </div>
                ...
                <form action="{{ route('distributor.update') }}" method="POST" id="editForm">
                    ...
                    <div class="row px-2">
                        <div class="form-group col-lg-6 mt-4 ">
                            ...
                            <input type="text" class="form-control h-75 rounded-4" id="Username"
                                value="{{ ucwords($distributor->user->name) }}" readonly>
                        </div>
                        <div class="form-group col-lg-6 mt-4 mr-auto">
                            <label for="name" class="form-label text-gray-800">Nama Toko</label>
                            <input type="text" class="form-control h-75 rounded-4" name="nama_toko" id="namaToko"
                                value="{{ ucwords($distributor->first()->nama_toko) }}" readonly>
                        </div>
                    </div>
                    <div class="row px-2">
                        <div class="form-group col-lg-6 mt-4 ">
                            <label for="name" class="form-label text-gray-800">Alamat</label>
                            <input type="text" class="form-control h-75 rounded-4" name="alamat" id="alamat"
                                value="{{ ucwords($distributor->first()->alamat) }}" readonly>
                        </div>
                        <div class="form-group col-lg-6 mt-4 mr-auto">
                            <label for="name" class="form-label text-gray-800">Nomor Telepon</label>
                            <input type="text" class="form-control h-75 rounded-4" name="no_hp" id="noHp"
                                value="{{ $distributor->first()->no_hp }}" readonly>
                        </div>
                    </div>
